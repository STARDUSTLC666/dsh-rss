...
const xmlParser = new XMLParser({
    ...
    parseTagValue: false,
    trimValues: true,
    isArray: (name) => name === 'item' || name === 'entry' || name === 'category' || name === 'link' || name === 'author' || name === 'contributor',
});
...
function pickRssEnclosure(item) {
...
/** RSS2 / RSS1 条目归一化。 */
function normalizeRssItem(item, baseUrl) {
...
/** Atom 条目归一化。 */
function normalizeAtomEntry(entry, baseUrl) {
...
/** 过滤掉既无标题又无链接的无效条目。 */
function keepUsable(entry) {
    return entry.title !== '' || entry.link !== '';
}
/** RSS2 / RSS1 频道元信息。 */
function buildRssFeedMeta(url, channel, feedType) {
    return {
        url,
...
/** Atom feed 元信息。 */
function buildAtomFeedMeta(url, feedRec) {
...
function detectTruncated(xmlText) {
    const head = xmlText.slice(0, 512).toLowerCase();
    if (/<feed[\s>]/.test(head) && !/<\/feed\s*>/i.test(xmlText))
        return 'feed';
    if (/<rdf:rdf[\s>]/.test(head) && !/<\/rdf:rdf\s*>/i.test(xmlText))
        return 'rdf:RDF';
    if (/<rss[\s>]/.test(head) && !/<\/rss\s*>/i.test(xmlText))
        return 'rss';
    return null;
}
/**
 ...
 */
export function parseFeed(xmlText, url) {
