...
export function parseOpml(text) {
    ...
    if (trimmed === '')
        throw new Error('OPML 内容为空，请提供 OPML XML 文本。');
    let doc;
    try {
    ...
    if (Object.keys(opml).length === 0)
        throw new Error('内容不是 OPML 文档（缺少 <opml> 根元素）。');
    const head = asRecord(opml.head);
    const body = asRecord(opml.body);
    const outlines = [];
    const walk = (node, inheritedCategory) => {
    ...
    for (const child of bodyOutlines)
        walk(child, '');
...
export function importOpmlFeeds(current, document) {
    const feeds = [...current];
    const added = [];
    const skipped = [];
    let existedCount = 0;
