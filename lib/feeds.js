 ...
 */
export function parseFeedsYaml(text) {
 ...
 */
export function addFeed(feeds, url, name, category) {
    ...
    const added = { url: cleanUrl, name: cleanName, category: cleanCategory };
 ...
 * 两者都缺或没有匹配项时抛中文错误。
 */
export function removeFeed(feeds, url, name) {
    const cleanUrl = (url ?? '').trim();
    const cleanName = (name ?? '').trim();
...
/** 按名字找订阅（不区分大小写）。 */
export function findFeedsByName(feeds, name) {
