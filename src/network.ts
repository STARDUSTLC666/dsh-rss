...
export type DnsLookupLike = (hostname: string) => Promise<readonly { address: string; family: number }[]>
...
function mergedSignal(timeoutMs: number, callerSignal?: AbortSignal): AbortSignal {
  const timeoutSignal = AbortSignal.timeout(timeoutMs)
  return callerSignal === undefined ? timeoutSignal : AbortSignal.any([callerSignal, timeoutSignal])
}

async function lookupWithSignal(hostname: string, lookupImpl: DnsLookupLike, signal: AbortSignal): Promise<readonly { address: string; family: number }[]> {
  signal.throwIfAborted()
  return await new Promise((resolveLookup, reject) => {
    const onAbort = () => reject(signal.reason)
    signal.addEventListener('abort', onAbort, { once: true })
    void Promise.resolve().then(() => lookupImpl(hostname)).then(
      (addresses) => {
...
export async function fetchFeedXml(
  url: string,
  cfg: ResolvedRssConfig,
  exec: RssToolExecution | undefined,
  fetchImpl?: FetchLike,
